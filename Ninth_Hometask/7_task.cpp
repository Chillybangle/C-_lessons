    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if(event.type == sf::Event::MouseButtonPressed) 
            {
                ...
                myball.charge = sign * myball.radius * myball.radius;
                myball.position = {(float)(event.mouseButton.x), (float)(event.mouseButton.y)};
                myball.velocity = {(float)(0), (float)(0)};
        ...
        for (int i = 0; i < balls_num; i++)
        {
            ...
            for (int j = 0; j < balls_num; ++j) 
            {
                ...
                float distance = std::sqrt((balls[j].position.x - balls[i].position.x) * (balls[j].position.x - balls[i].position.x) +
                                           (balls[j].position.y - balls[i].position.y) * (balls[j].position.y - balls[i].position.y)); 
